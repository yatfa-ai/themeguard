...
describe("runCli — the exit code moves with the sixth rule, both ways", () => {
  it("a loop is exit 1, its own section, and its count on the summary line", () => {
    ...
    expect(result.stdout).toContain(
      "1 finding: 0 collision, 0 dead-token, 0 scale-collapse, 0 family-consistency, 0 unresolved-reference, 1 cycle-reference, 0 duplicate-declaration, 0 unresolved-import, 0 theme-partial-token.",
    );
  });
