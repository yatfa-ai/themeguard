...
describe("the manifest, before anything is packed", () => {
  it("is version 0.1.9 — 0.1.6 added the fifth rule, unresolved-reference; 0.1.7 takes one invocation over several stylesheets; 0.1.9 adds the sixth rule, cycle-reference: a var() loop is a defect, judged from the resolver's kind:\"cycle\" chains", () => {
...
describe("a project that has installed the package", () => {
  ...
  it("resolves the library through the exports map at RUNTIME", () => {
    ...
    expect(sh(process.execPath, [script], consumer).trim()).toBe(
      '{"collision":11,"dead-token":2,"scale-collapse":2,"family-consistency":7,"unresolved-reference":0,"cycle-reference":0}',
    );
  });
...
describe("publication readiness", () => {
   ...
   */
  it("passes `npm publish --dry-run` and reports the 0.1.9 tarball", () => {
    ...
    expect(combined).toContain("themeguard@0.1.9");
  }, 300_000);
});
