...
describe("parseConfig — what the package can honour", () => {
  ...
  it("rejects an unknown rule id, naming the entry and the known rules", () => {
    ...
    expect(error.message).toContain("collision, dead-token, scale-collapse, family-consistency, unresolved-reference, cycle-reference");
  });
...
describe("themeguard <file.css> with themeguard.config.json beside the stylesheet", () => {
  it("absent config: the full census, exit 1, and the suppressed section headlines at zero", () => {
    ...
    expect(result.stdout).toContain("22 findings: 11 collision, 2 dead-token, 2 scale-collapse, 7 family-consistency, 0 unresolved-reference, 0 cycle-reference.");
  ...
  it("suppressing the fixture's two deliberate hovers: scale-collapse (0), a named suppressed section with quoted reasons, and 20 findings on the total line", () => {
    ...
    expect(result.stdout).toContain(
      "20 findings: 11 collision, 2 dead-token, 0 scale-collapse, 7 family-consistency, 0 unresolved-reference, 0 cycle-reference.",
    );
    ...
    expect(
      result.out.filter((l) => /^ {2}\[(collision|dead-token|scale-collapse|family-consistency|unresolved-reference|cycle-reference)\]/.test(l)),
    ).toHaveLength(20);
  });
  ...
  it("a theme-scoped entry suppresses only its theme's finding: the other theme's finding still reports and the exit stays 1", () => {
    ...
    expect(result.stdout).toContain("suppressed (1)");
    expect(result.stdout).toContain("1 finding: 1 collision, 0 dead-token, 0 scale-collapse, 0 family-consistency, 0 unresolved-reference, 0 cycle-reference.");
