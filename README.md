...
| `src/resolve.ts` | What each property resolves to **per theme**, following `var()` chains — through embedded primary-position references (`1px solid var(--c)`) as well as whole-value ones, and re-marking a walk that stopped at a compound value `cycle` when the value it stopped at references a loop MEMBER in that theme's view — iterated to a fixed point, so a dependent several hops out is reached too (the dependent-declaration half of the same defect). Theme absence, translucency, unresolved references and cycles are each represented explicitly — none of them is an error and none is guessed at. |
...
| `cycle-reference` | — | 0 | Nothing to filter on this fixture — no `var()` chain in it returns to a name already on the chain, so there are no candidates at all. Its four compound-with-`var()` declarations (the `color-mix` toast surfaces) all point at plain non-ancestor colours, so the compound back-edge scan reads them and finds nothing — and the dependent-declaration pass that re-marks compound-stopped walks reads the same finished results, reaches its fixed point on the first round, and finds nothing to re-mark. The tests pin the rule's populations on hostile sheets (a root loop, a self-loop, a theme-authored loop, a compound loop, the override-heals and inherited-dedupe negatives, a tail into a compound loop, a compound tail into a loop, and a dependent several hops out) and pin this 0 as a preservation check. |
