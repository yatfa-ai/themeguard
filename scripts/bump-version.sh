...
#
# Mirrors warden's scripts/bump-version.sh (which mirrors yatfa's
# script/bump-version.sh), adapted for a plain npm package:
#   - version source is package.json (npm reads it), not a VERSION file
#   - release is tag-based (GitHub Release attaches to vX.Y.Z); themeguard has
#     no `prod` deploy-mirror branch — npm IS the distribution, so the
#     prod-branch push from yatfa/specguard is dropped and nothing advances
#     beyond main + tag. The publish job builds from the tag this script made.
#
...
# Bump patch version (0.1.15 -> 0.1.16)
...
# (e.g. reflow the version-notes "//" comment) and bloat the bump commit.
